...
const SSInput = <T extends FieldValues>({
  ...
  return (
    <div className="w-full max-w-full flex flex-col box-border">
      ...
      <div className="relative w-full max-w-full flex items-center box-border rounded-xl overflow-hidden">
        {icon && (
        ...
        <input
          ...
          className={`w-full h-11 block box-border rounded-xl border bg-slate-900/40 dark:bg-slate-900/60 text-sm transition-all duration-200 focus:outline-none focus:ring-2 ${
            ...
            error
              ...
              : "border-slate-700 dark:border-slate-700/80 text-slate-100 dark:text-slate-200 focus:border-blue-500 focus:ring-blue-500/20"
          }`}
        />
      ...
      {error && (
        <p className="text-xs font-semibold text-rose-400 mt-1.5 text-left w-full break-words overflow-hidden" aria-live="assertive">
          {error.message}
        </p>
      )}
    </div>
